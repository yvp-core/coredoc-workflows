...
Two checks precede the verdict. First, refresh the recorded ids by exact id and
report per-id version changes, a `supersededById` successor, and any replacement
candidate. Second, rediscover against the ACTUAL diff: map the changed
files/symbols to stable node ids through the graph and spend the stage's single
broad lookup on `nodeIds` of those symbols, narrowed by `domain`/`feature` when
the working set already named one; retain the routed set alongside what comes
back, and name unmapped files and evidence gaps separately from server
truncation. A server snapshot may predate unpushed changes — say so; publishing
is never a precondition for this advisory check. When no intent capability is
...
visible — also prepare the "Anchor suggestions" list from the write stage of
`<plugin-root>/resources/methodology/intent-context.md`, and never review,
anchor, or record anything on your own judgement. When no intent capability is
present, proceed from repository evidence alone and do not mention intent context
in the output.
...
touches whose status is not `matched`; any truncation seen; and the delivery line
from the methodology's Release bullet. With a write capability present, the
"Anchor suggestions" list goes here too, as suggestions and not as executed
anchors.
