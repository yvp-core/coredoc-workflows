...
Before the verdict, use the cloud task-context call for the ACTUAL diff: task
text, changed `files: [{repoKey, path}]`, known node IDs, and the routed intent
IDs together, with a known domain/feature when applicable. Compare routed
versions and report successors or replacement candidates. On local/legacy
surfaces, follow the methodology's exact-ID-first protocol and bounded node
lookup. Retain the routed set alongside new constraints; name unmapped files,
truncation and freshness gaps. A snapshot may predate unpushed changes — say
so; publishing is never a precondition for this advisory check. When no intent capability is
...
the implementation bindings and prepare the PR trailer handoff. Keep this
...
When intent context was used, add a **Reviewed intent** block: the rules that
influenced the implementation; the rules retrieved and judged not applicable,
each with its reason; rules newly discovered against the diff; anchors the change
touches whose status is not `matched`; and any truncation seen. Include suggested
PR-scoped Coredoc-Intent-Anchors locators and explicit replacements when
implementation mapping changes. Carry the exact working set; do not rediscover
IDs or guess node IDs. Save the structured temporary handoff and validate with
the bundled intent-anchor-block command before passing it to PR-writing.
Review remains read-only for source, KB and PR; do not execute anchors.
...
Carry the trailer block to the next authorized PR-writing stage. Review alone
does not authorize a PR body update, even when `gh` and a pull request exist.
The authorized writer applies the block using the methodology's Release
instructions; do not require the user to paste it manually. Coredoc records plan
and delivery from those lines in `merge` or `deploy` mode; in `manual` mode
recording availability stays the maintainer's own `intent_release` action.

```text
Coredoc-Intent-Delivers: cap-widget-ordering@3, br-refund-window@2
Coredoc-Intent-Retires: lim-old-export-cap@4
```
...
node. Do not add a routine anchor-approval questionnaire to review; CI resolves
the reviewed PR mapping block after graph publication.
