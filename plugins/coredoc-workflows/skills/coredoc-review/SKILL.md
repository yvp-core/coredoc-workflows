...
changes to `.coredoc/intent-bindings.json` when the implementation mapping is
missing or wrong. Review is read-only: report the suggested diff in the handoff;
do not write the manifest or execute anchors.
...
the reviewed bindings manifest after graph publication.
