...
Review only the requested plan/spec/path and remain read-only. Repository rules
and the user's authorization boundary win. Ground claims in source and read-only
Coredoc graph evidence when available; graph coverage is a lower bound.
...
applicable intent IDs next to the claims they support. When no intent capability
...
Update the reviewed artifact only when authorized. Preserve intent IDs and map
every step to an accepted outcome; remove steps with no current consumer or
observer. Use a single small Mermaid diagram only for a non-trivial dependency,
state, or interaction flow, and do not duplicate it in prose.
...
Record accepted decisions in the spec's ADR/decision section; unresolved
decisions retain an owner and one concrete question. Propose TODO updates only
for accepted nonblocking follow-ups, and ask before writing them.
...
4. reachable failure modes, rollout, rollback, and public consumers are covered;
