...
**Canonical pattern catalog.** The plugin ships the detection taxonomy — 3 tiers
(HIGH credentials, MEDIUM PII/legal/internal, LOW hygiene) with per-pattern
validators. Run it rather than matching prefixes by eye:

```bash
<plugin-root>/bin/coredoc-workflows redact-scan --repo-emails <path...>   # or --stdin
<plugin-root>/bin/coredoc-workflows redact-scan --tier HIGH <path...>
```

Pass `--repo-emails` when scanning a repository: it suppresses addresses already
public there (git identity, recent commit authors, CODEOWNERS), which are noise
...
It exits 2 when anything HIGH is found, 0 otherwise, and masks every matched span
...
have `aws_secret_access_key` within 100 characters, a card number must pass Luhn,
an env assignment must clear an entropy floor, an IPv4 must be outside every
private range, and a span that is itself a placeholder (AWS's own
`AKIA…EXAMPLE`, `<your-key>`) is suppressed. Keep using the history greps for
what the scanner cannot see — deleted blobs and past revisions.
...
- `VERIFIED` — deterministic source tracing or safe testing establishes the
  trigger, reachable path, and security outcome (7-10 confidence). Verification
  status does not itself pass the daily report gate: an ordinary 7/10 item
  remains suppressed in daily mode.
- `UNVERIFIED` — factual evidence, reachability, or outcome remains incomplete
  (5-6 confidence; comprehensive mode only).
- `TENTATIVE` — speculation or a pattern match retained by comprehensive mode
  (2-4 confidence).
- `NEEDS_CONTEXT` — the security defect is source-proven at 7-10 confidence, but
  exactly one missing release-policy fact could change its severity, blocking,
...
Resolve verifier breadth from the effective Review policy before dispatch. First
deduplicate candidates by root cause and group related candidates by trust
boundary. The generic fallback uses the smallest useful set and one independent
verifier for the material security candidates; a repository policy may require
broader domain expertise or additional independent passes. Never create one
verification task per finding merely because multiple candidates exist.

Keep each verifier independent. Do not provide the initial title, severity,
confidence, exploit narrative, or scanner rationale; the verifier must
reconstruct any claim from code rather than rubber-stamp an anchored conclusion.
...
- "Read the cited code and reachable callers independently. Determine whether a
  ...
  threshold, name the missing proof; do not call the candidate unreal unless
  evidence disproves it."

Read and apply
`<plugin-root>/resources/methodology/subagent-dispatch.md` when scheduling the
authorized verifier set. Concurrency only batches work; it never increases
...
For a source-proven item whose one missing release-policy fact could change the
verdict, use the same main findings section with:

```md
* **Severity:** CRITICAL | HIGH | MEDIUM, or `null` when the missing fact could change severity
* **Confidence:** N/10 based only on factual evidence
