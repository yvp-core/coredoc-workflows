# Security methodology acknowledgements

The audit method shipped by `coredoc-security-review` descends from upstream's
`/cso` v2, which was informed by research across the security audit landscape.
Those credits are carried forward here — the method is ours now, the ideas in it
are not. Credits to:
